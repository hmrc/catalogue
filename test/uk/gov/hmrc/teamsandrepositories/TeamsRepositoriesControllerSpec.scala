...
class TeamsRepositoriesControllerSpec extends PlaySpec with MockitoSugar with Results with OptionValues with OneServerPerSuite with Eventually {
  ...
  val defaultData =
    Seq(
      new TeamRepositories("test-team", List(
                    GitRepository("repo-name", "some description", "repo-url", createdDate = createdDateForService1, lastActiveDate = lastActiveDateForService1, repoType = RepoType.Service, digitalServiceName = Some("digital-service-2"), language = Some("Scala"), lastSuccessfulScheduledUpdate = None),
                    GitRepository("library-repo", "some description", "library-url", createdDate = createdDateForLib1, lastActiveDate = lastActiveDateForLib1, repoType = RepoType.Library, digitalServiceName = Some("digital-service-3"), language = Some("Scala"), lastSuccessfulScheduledUpdate = None)
                  ), System.currentTimeMillis()),
      new TeamRepositories("another-team", List(
                    GitRepository("another-repo", "some description", "another-url", createdDate = createdDateForService2, lastActiveDate = lastActiveDateForService2, repoType = RepoType.Service, digitalServiceName = Some("digital-service-1"), language = Some("Scala"), lastSuccessfulScheduledUpdate = None),
                    GitRepository("middle-repo", "some description", "middle-url", createdDate = createdDateForService3, lastActiveDate = lastActiveDateForService3, repoType = RepoType.Service, digitalServiceName = Some("digital-service-2"), language = Some("Scala"), lastSuccessfulScheduledUpdate = None),
                    GitRepository("alibrary-repo", "some description", "library-url", createdDate = createdDateForLib2, lastActiveDate = lastActiveDateForLib2, repoType = RepoType.Library, digitalServiceName = Some("digital-service-1"), language = Some("Scala"), lastSuccessfulScheduledUpdate = None),
                    GitRepository("CATO-prototype", "some description", "prototype-url", createdDate = createdDateForLib2, lastActiveDate = lastActiveDateForLib2, repoType = RepoType.Prototype, digitalServiceName = Some("digital-service-2"), language = Some("Scala"), lastSuccessfulScheduledUpdate = None),
                    GitRepository("other-repo", "some description", "library-url", createdDate = createdDateForLib2, lastActiveDate = lastActiveDateForLib2, repoType = RepoType.Other, digitalServiceName = Some("digital-service-1"), language = Some("Scala"), lastSuccessfulScheduledUpdate = None)
                  ), System.currentTimeMillis())
    )
  ...
  def singleRepoResult(teamName: String = "test-team", repoName: String = "repo-name", repoUrl: String = "repo-url", isInternal: Boolean = true) = {
    Seq(
      new TeamRepositories("test-team", List(
                    GitRepository(repoName, "some description", repoUrl, createdDate = now, lastActiveDate = now, isInternal = isInternal, repoType = RepoType.Service, language = Some("Scala"), lastSuccessfulScheduledUpdate = None)), System.currentTimeMillis()))
  }
  ...
  "Retrieving a list of repositories for a team" should {
    ...
    "Return information about all the teams that have access to a repo" in {
      val sourceData =
        Seq(
          new TeamRepositories("test-team", List(GitRepository("repo-name", "some description", "repo-url", createdDate = now, lastActiveDate = now, repoType = RepoType.Service, language = Some("Scala"), lastSuccessfulScheduledUpdate = None)), System.currentTimeMillis()),
          new TeamRepositories("another-team", List(GitRepository("repo-name", "some description", "repo-url", createdDate = now, lastActiveDate = now, repoType = RepoType.Service, language = Some("Scala"), lastSuccessfulScheduledUpdate = None)), System.currentTimeMillis())
        )
    ...
    "not show the same service twice when it has an open and internal source repository" in {
      val sourceData =
        Seq(new TeamRepositories("test-team", List(
                          GitRepository("repo-name", "some description", "Another-url", createdDate = now, lastActiveDate = now, repoType = RepoType.Service, language = Some("Scala"), lastSuccessfulScheduledUpdate = None),
                          GitRepository("repo-name", "some description", "repo-url", createdDate = now, lastActiveDate = now, repoType = RepoType.Service, language = Some("Scala"), lastSuccessfulScheduledUpdate = None),
                          GitRepository("aadvark-repo", "some description", "aadvark-url", createdDate = now, lastActiveDate = now, repoType = RepoType.Service, language = Some("Scala"), lastSuccessfulScheduledUpdate = None)), System.currentTimeMillis()))
  ...
  "Retrieving a list of repository details for a team" should {
    ...
    "Return the repository information for the specified team" in {
      val sourceData =
        Seq(
          TeamRepositories("test-team", List(GitRepository("repo-name", "some description", "repo-url", createdDate = now, lastActiveDate = now, repoType = RepoType.Service, language = Some("Scala"), lastSuccessfulScheduledUpdate = None)), System.currentTimeMillis()),
          TeamRepositories("another-team", List(GitRepository("repo-name", "some description", "repo-url", createdDate = now, lastActiveDate = now, repoType = RepoType.Service, language = Some("Scala"), lastSuccessfulScheduledUpdate = None)), System.currentTimeMillis())
        )
  ...
  "Retrieving a list of all services" should {
    ...
    "Ignore case when sorting alphabetically" in {
      val sourceData =
        Seq(TeamRepositories("test-team", List(
                          GitRepository("Another-repo", "some description", "Another-url", createdDate = now, lastActiveDate = now, repoType = RepoType.Service, language = Some("Scala"), lastSuccessfulScheduledUpdate = None),
                          GitRepository("repo-name", "some description", "repo-url", createdDate = now, lastActiveDate = now, repoType = RepoType.Service, language = Some("Scala"), lastSuccessfulScheduledUpdate = None),
                          GitRepository("aadvark-repo", "some description", "aadvark-url", createdDate = now, lastActiveDate = now, repoType = RepoType.Service, language = Some("Scala"), lastSuccessfulScheduledUpdate = None)), System.currentTimeMillis()))
    ...
    "Treat as one service if an internal and an open repo exist" in {
      val sourceData =
        Seq(
          new TeamRepositories("test-team", List(
                                GitRepository("repo-name", "some description", "repo-url", createdDate = now, lastActiveDate = now, isInternal = true, repoType = RepoType.Service, language = Some("Scala"), lastSuccessfulScheduledUpdate = None),
                                GitRepository("repo-name", "some description", "repo-open-url", createdDate = now, lastActiveDate = now, isInternal = false, repoType = RepoType.Service, language = Some("Scala"), lastSuccessfulScheduledUpdate = None)), System.currentTimeMillis()))
    ...
    "return the empty list for repository type if a team does not have it" in {
      ...
      val sourceData =
        Seq(
          new TeamRepositories("test-team", List(
                                GitRepository("repo-name", "some description", "repo-url", createdDate = now, lastActiveDate = now, isInternal = true, repoType = RepoType.Library, language = Some("Scala"), lastSuccessfulScheduledUpdate = None),
                                GitRepository("repo-open-name", "some description", "repo-open-url", createdDate = now, lastActiveDate = now, isInternal = false, repoType = RepoType.Library, language = Some("Scala"), lastSuccessfulScheduledUpdate = None)), System.currentTimeMillis()))
