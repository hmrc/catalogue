...
class TeamRepositoryWrapperSpec extends WordSpec with Matchers {
  ...
  "asTeamList" should {
    ...
    "calculate activity dates based on min of created and max of last active when there are multiple versions of the same repo" in {
      ...
      val teams = Seq(
        PersistedTeamAndRepositories("teamNameChicken", List(newDeployableRepo, oldestLibraryRepo)),
        PersistedTeamAndRepositories("teamName", List(oldDeployableRepo, oldOtherRepoWithLatestActiveDate)),
        PersistedTeamAndRepositories("teamNameNotActive", List())
      )
    ...
    "Exclude specified repos in calculating activity max and min dates" in {
      ...
      val teams = Seq(
        PersistedTeamAndRepositories("teamNameChicken", List(oldLibraryRepo, newDeployableRepo, newIgnoreRepo)),
        PersistedTeamAndRepositories("teamName", List(oldDeployableRepo, newLibraryRepo, newIgnoreRepo)),
        PersistedTeamAndRepositories("teamNameNotActive", List())
      )
  ...
  "asServiceRepoDetailsList" should {
    ...
    "include repository with type not Deployable as services if one of the repositories with same name is Deployable" in {
      ...
      val teams = Seq(
        PersistedTeamAndRepositories("teamName", List(
        ...
        PersistedTeamAndRepositories("teamNameOther", List(
          GitRepository("repo3", "some desc", "", isInternal = true, repoType = RepoType.Library, createdDate = timestamp, lastActiveDate = timestamp))
        )
      )
    ...
    "calculate activity dates based on min of created and max of last active when there are multiple versions of the same repo" in {
      ...
      val teams = Seq(
        PersistedTeamAndRepositories("teamNameChicken", List(oldestLibraryRepo)),
        PersistedTeamAndRepositories("teamName", List(oldDeployableRepo, newDeployableRepo)),
        PersistedTeamAndRepositories("teamNameOther", List(newestOtherRepo))
      )
    ...
    "asLibraryRepoDetailsList" should {
      "not include libraries if one of the repository with same name is Deployable" in {
        ...
        val teams = Seq(
          ...
          PersistedTeamAndRepositories("teamNameOther", List(GitRepository("repo4", "Some description", "", isInternal = true, repoType = RepoType.Library, createdDate = timestamp, lastActiveDate = timestamp)))
        )
        val wrapper: TeamRepositoryWrapper = new TeamRepositoryWrapper(teams)
    ...
    "findRepositoryDetails" should {
      ...
      "find repository as type Library even if one of the repo with same name is not type library" in {
        val teams = Seq(
          ...
          PersistedTeamAndRepositories("teamNameOther1", List(GitRepository("repo1", "Some description", "", isInternal = false, repoType = RepoType.Library, createdDate = timestamp, lastActiveDate = timestamp)))
        )
        val wrapper: TeamRepositoryWrapper = new TeamRepositoryWrapper(teams)
        val result: Option[RepositoryDetails] = wrapper.findRepositoryDetails("repo1", UrlTemplates(Seq(), Seq(), Map()))
      ...
      "not include repository with prototypes in their names" in {
        val teams = Seq(
          ...
          PersistedTeamAndRepositories("teamNameOther", List(GitRepository("repo3", "Some description", "", isInternal = true, repoType = RepoType.Other, createdDate = timestamp, lastActiveDate = timestamp)))
        )
    ...
    "asTeamRepositoryNameList" should {
      ...
      "include repository with type not Deployable as services if one of the repository with same name is Deployable" in {
        val teams = Seq(
          ...
          PersistedTeamAndRepositories("teamNameOther", List(GitRepository("repo3", "Some description", "", isInternal = true, repoType = RepoType.Library, createdDate = timestamp, lastActiveDate = timestamp)))
        )
        val wrapper: TeamRepositoryWrapper = new TeamRepositoryWrapper(teams)
    ...
    "asRepositoryTeamNameList" should {
      ...
      "group teams by services they own filtering out any duplicates" in {
        ...
        val teams = Seq(
          PersistedTeamAndRepositories("team1", List(
            GitRepository("repo1", "Some description", "", isInternal = false, repoType = RepoType.Deployable, createdDate = timestamp, lastActiveDate = timestamp),
          ...
          PersistedTeamAndRepositories("team2", List(
            GitRepository("repo2", "Some description", "", isInternal = true, repoType = RepoType.Library, createdDate = timestamp, lastActiveDate = timestamp),
            GitRepository("repo3", "Some description", "", isInternal = true, repoType = RepoType.Library, createdDate = timestamp, lastActiveDate = timestamp))),
          PersistedTeamAndRepositories("team2", List(
            GitRepository("repo2", "Some description", "", isInternal = true, repoType = RepoType.Library, createdDate = timestamp, lastActiveDate = timestamp),
            GitRepository("repo3", "Some description", "", isInternal = true, repoType = RepoType.Library, createdDate = timestamp, lastActiveDate = timestamp))),
          PersistedTeamAndRepositories("team3", List(
            GitRepository("repo3", "Some description", "", isInternal = true, repoType = RepoType.Library, createdDate = timestamp, lastActiveDate = timestamp),
    ...
    "asTeamRepositoryDetailsList" should {
      ...
      "Include all repository types when get the max last active and min created at for team" in {
        ...
        val teams = Seq(
          PersistedTeamAndRepositories("teamName", List(oldDeployableRepo, newLibraryRepo, newOtherRepo)),
          PersistedTeamAndRepositories("teamNameOther", List(GitRepository("repo3", "Some description", "", isInternal = true, repoType = RepoType.Library, createdDate = timestamp, lastActiveDate = timestamp)))
        )
      ...
      "Exclude all shared repositories when calculating the min and max activity dates for a team" in {
        ...
        val teams = Seq(
          PersistedTeamAndRepositories("teamName", List(oldDeployableRepo, newLibraryRepo, newOtherRepo, sharedRepo)),
          PersistedTeamAndRepositories("teamNameOther", List(GitRepository("repo3", "Some description", "", isInternal = true, repoType = RepoType.Library, createdDate = timestamp, lastActiveDate = timestamp)))
        )
      ...
      "populate firstServiceCreation date by looking at only the service repository" in {
        ...
        val teams = Seq(
          PersistedTeamAndRepositories("teamName", List(newDeployableRepo, oldDeployableRepo, newLibraryRepo, newOtherRepo, sharedRepo)),
          PersistedTeamAndRepositories("teamNameOther", List(GitRepository("repo3", "Some description", "", isInternal = true, repoType = RepoType.Library, createdDate = timestamp, lastActiveDate = timestamp)))
        )
